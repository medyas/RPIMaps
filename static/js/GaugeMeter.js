...
!function ($) {
    $.fn.gaugeMeter = function (t) {
		...
		// removed cause it's causing undefined function error in the webkit
            //if(Number.isInteger(parseInt(option.used)) && Number.isInteger(option.total)){
            ...
            var b = $("<canvas>not supported!</canvas>").attr({width: option.size, height: option.size}).get(0),
                    g = b.getContext("2d"),
                    m = b.width / 2,
                    v = b.height / 2,
